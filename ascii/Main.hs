...
gameLoopState::(Int->IO (Maybe GameField)) -> StateT Game IO ()
gameLoopState levelReader = do
              ...
              newGame <- if finishLevel then do
                                          ...
                                          if isNothing nextLevel
                                            then do liftIO $ putStrLn "all levels won"
                                                    liftIO exitSuccess
                                            else return $ game & field  .~ fromJust nextLevel
                                                        & level +~ 1
                                        else
                                          case c of
                                            'q' -> return game
